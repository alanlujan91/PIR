...
vars_decision = [
    "Q26v2",  ## spending growth or decrease next year
    "Q26v2part2",
...
vars_macroexp = ["Q6new", "Q4new"]  ## stock market going up  ## UE goes up
...
# + {"code_folding": []}
SCEM["age"] = SCEM["age"].astype("int", errors="ignore")
...
for col in ["HHinc", "age", "educ", "HHinc_gr", "educ_gr", "nlit_gr"]:
    SCEM[col] = SCEM[col].astype("int", errors="ignore")
...
SCEM.replace(cleanup_nums, inplace=True)

# + {"code_folding": []}
## create age group

SCEM["age_gr"] = pd.cut(
    SCEM["age"], 5, labels=["20-30", "30-39", "40-48", "49-57", ">57"]
)
...
SCEM["byear_gr"] = pd.cut(
    SCEM["byear"], 6, labels=["40s", "50s", "60s", "70s", "80s", "90s"]
)
...
non_categorical = [
    var for var in SCEM.columns if var not in vars_cat + ["age_gr", "byear_gr"]
]
...
for i, mom in enumerate(moms):
    ...
    if i == len(moms) - 1:
        axes[i].set_xlabel("group by household income (from low to high)", size=15)
plt.tight_layout()
...
for i, mom in enumerate(moms):
    ...
    if i == len(moms) - 1:
        axes[i].set_xlabel("group by education (from low to high)", size=15)
plt.tight_layout()
...
for i, mom in enumerate(moms):
    ...
    if i == len(moms) - 1:
        axes[i].set_xlabel("group by gender", size=15)
plt.tight_layout()
...
for i, mom in enumerate(moms):
    ...
    if i == len(moms) - 1:
        axes[i].set_xlabel("group by numeracy", size=15)
plt.tight_layout()
...
for mom_id, mom in enumerate(mom_list):
    if mom != "incskew":
        ...
        mom_lb, mom_ub = (
            np.percentile(mom_nonan, 2),
            np.percentile(mom_nonan, 98),
        )  ## exclude top and bottom 3% observations
        to_keep = (mom_nonan < mom_ub) & (mom_nonan > mom_lb) & (mom_nonan != 0)
        to_plot = mom_nonan[to_keep]

    fig, ax = plt.subplots(figsize=(8, 6))
    sns.histplot(
        data=to_plot, kde=True, stat="density", color="red", bins=40, alpha=0.3
    )
...
lb, ub = (
    np.percentile(individual_risk, 10),
    np.percentile(individual_risk, 70),
)  ## exclude top and bottom 3% observations
to_keep = (individual_risk < ub) & (individual_risk != 0)
individual_risk_keep = individual_risk[to_keep]

sns.histplot(
    data=individual_risk_keep,
    kde=True,
    color="blue",
    bins=60,
    stat="density",
...
lb, ub = (
    np.percentile(individual_risk, 10),
    np.percentile(individual_risk, 90),
)  ## exclude top and bottom 3% observations
to_keep = (individual_risk < ub) & (individual_risk != 0)
individual_risk_keep = individual_risk[to_keep]
...
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.xlabel("PRs and Predicted Volatility in std", fontsize=15)
plt.legend(loc=0)
...
print("Mean of nominal risk perception is " + str(round(mean_std, 3)) + " in stv")

med_std = np.sqrt(SCEM["incvar"].median())
print("Median of nominal risk perception is " + str(round(med_std, 3)) + " in stv")
...
print("Mean of risk perception is " + str(round(mean_std, 3)) + " in stv")

med_std = np.sqrt(SCEM["rincvar"].median())
print("Median of risk perception is " + str(round(med_std, 3)) + " in stv")
...
print("10/90 IQR of risk perception is " + str(round(riqr, 3)) + " in stv")

iqr = SCEM["incstd_rd"].quantile(q=0.9) - SCEM["incstd_rd"].quantile(q=0.1)
print("10/90 IQR of nominal risk perception is " + str(round(iqr, 3)) + " in stv")
...
for mom_id, mom in enumerate(mom_list):
    ...
    fig, ax = plt.subplots(figsize=(8, 6))
    sns.histplot(
        data=to_plot, kde=True, stat="density", color="gray", bins=60, alpha=0.3
    )
    plt.xticks(fontsize=14)
    plt.yticks(fontsize=14)
...
for i, mom in enumerate(dep_list):
    ...
    model1 = smf.ols(formula=str(mom) + "~ age+age2+age3", data=SCEM)
    rs_list[nb_spc * i + 0] = model1.fit()

    ## model 2 experienced vol, age, income, education

    model2 = smf.ols(
        formula=str(mom) + "~ age+age2+age3 + C(gender)+ C(nlit_gr)", data=SCEM
    )
    ...
    model3 = smf.ols(
        formula=str(mom) + "~ age+age2+age3+C(gender)+ C(nlit_gr)+C(educ_gr)", data=SCEM
    )
...
for i, mom in enumerate(dep_list2):
    ## model 1
    model = smf.ols(formula=str(mom) + "~ C(parttime)+C(selfemp)", data=SCEM)
    rs_list[nb_spc * i] = model.fit()

    ## model 2
    ct_str = "+".join([var for var in indep_list_ct])
    model2 = smf.ols(
        formula=str(mom) + "~ age+age2+age3+C(parttime)+C(selfemp) + " + ct_str,
        data=SCEM,
    )
    rs_list[nb_spc * i + 1] = model2.fit()
...
for i, mom in enumerate(dep_list3):
    ## model 1
    model = smf.ols(formula="spending~" + "+" + mom, data=SCEM)
    rs_list[nb_spc * i] = model.fit()
...
dfoutput = summary_col(
    ...
    float_format="%0.2f",
    stars=True,
    regressor_order=["incexp", "incvar", "rincvar", "incskew", "UEprobAgg"],
    info_dict={
        "N": lambda x: "{0:d}".format(int(x.nobs)),
        "R2": lambda x: "{:.2f}".format(x.rsquared),
    },
