...
lc_data = [
    ...
    (
        "z_val",
        float64[:],
    ),  # values of z, grid values for the continuous (macro) persistent state    x
    ("sigma_psi", float64),  # permanent shock volatility              x
    ("sigma_eps", float64),  # transitory shock volatility
    (
        "x",
        float64,
    ),  # MA(1) coefficient, or essentially the autocorrelation coef of non-permanent income
    ("b_y", float64),  # loading of macro state to income        x
    (
        "borrowing_cstr",
        boolean,
    ),  ## artificial borrowing constraint if True, natural borrowing constraint if False
    ...
    (
        "bequest_ratio",
        float64,
    ),  ## zero: bequest thrown to the ocean; one: fully given to newborns
    ...
    (
        "eps_grid",
        float64[:],
    ),  # Exogenous grid over transitory income shocks (for ma only)
...
class LifeCycle:
    ...
    def __init__(
        ...
        P_sub=np.array([[0.5, 0.5], [0.5, 0.5]]),
        ...
        ## saving a grid
        a_grid_regular = np.exp(
            np.linspace(np.log(1e-6), np.log(grid_max), grid_size - 1)
        )
        ...
        if sigma_eps != 0.0 and x != 0.0:
            ...
            self.eps_grid = np.array(
                [0.0, 0.001]
            )  ## make two points for the c function to be saved correctly
    ...
    def prepare_shocks(self):
        ...
        self.psi_shk_draws = np.log(
            psi_shk_dist.X
        )  ## the discretized is lognormal variable itself, we work with the log of it
        ...
        self.psi_shk_mkv_draws = np.stack(
            (np.log(sigma_psi_2mkv_l.X), np.log(sigma_psi_2mkv_h.X))
        )
        ...
        self.eps_shk_mkv_draws = np.stack(
            (np.log(sigma_eps_2mkv_l.X), np.log(sigma_eps_2mkv_h.X))
        )
        ...
        if self.q == 0.0:
            ...
            for z in range(n):
                for j in range(k2):
                    ...
                    ## m = a + c
                    # m_init[:,j,z] = self.a_grid
                    σ_init[:, j, z] = (self.q * self.a_grid ** (-self.ρ_b)) ** (
                        -1 / self.ρ
                    )
...
def EGM_combine(
    ...
    G = lc.G[
        age_id + 1
    ]  ## get the age specific growth rate, G[T] is the sudden drop in retirement from working age
                ...
                for z_hat in range(n):
                    ...
                    for eps_shk in eps_shk_draws:
                        for psi_shk in psi_shk_draws:
                            ...
                            if (
                                age <= lc.T - 1
                            ):  # till say 39, because consumption policy for t=40 changes
                                ## work
                                Y_hat = (
                                    (1 - λ) * (1 - λ_SS) * Y(z_val_hat, u_shk)
                                )  ## conditional on being employed
                                c_hat = σ(
                                    R / (G * Γ_hat) * a + Y_hat + transfer,
                                    eps_shk,
                                    z_hat,
                                )
...
@njit
def extrapolate(theta, x, eps_shk):
    """
...
@njit
def EGM_br(mϵ_in, σ_in, age_id, lc):
    """
...
def compare_2solutions(ms_stars, σs_stars):
    """
    input
    ======
...
if __name__ == "__main__":
    ...
    plt.plot(m_init[0:-1, 0, 1], σ_init[0:-1, 0, 1])
...
if __name__ == "__main__":
    ...
    ## solve backward
    ms_star_basic, σs_star_basic = solve_model_backward_iter(lc, m_init, σ_init)
...
if __name__ == "__main__":
    ...
    for i, sigma_psi in enumerate(sigma_psi_ls):
        ...
        ## solve backward
        ms_star, σs_star = solve_model_backward_iter(lc_pt, m_init, 0.5 * σ_init)
...
if __name__ == "__main__":
    ...
    for x, year in enumerate(years_left):
        ...
        for k, sigma_psi in enumerate(sigma_psi_ls):
            m_plt, c_plt = (
                ms_stars[k][i, 0:-1, eps_fix, 0],
                σs_stars[k][i, 0:-1, eps_fix, 0],
            )
            axes[x].plot(
                m_plt,
                c_plt,
...
if __name__ == "__main__":
    ...
    dem3d = ax.plot_surface(xx, yy, c_stars, cmap="viridis")
    ax.set_xlim(60, 0)
...
if __name__ == "__main__":
    ...
    for i, P in enumerate(P_ls):
        ...
        ## solve the model
        ms_star_ar, σs_star_ar = solve_model_backward_iter(lc_ar, m_init_ar, σ_init_ar)
...
if __name__ == "__main__":
    ...
    for x, year in enumerate(years_left):
        ...
        for eps in eps_ls:
            m_plt_l, c_plt_l = (
                ms_stars_ar[0][i, 0:-1, eps, 0],
                σs_stars_ar[0][i, 0:-1, eps, 0],
            )
            m_plt_h, c_plt_h = (
                ms_stars_ar[0][i, 0:-1, eps, 1],
                σs_stars_ar[0][i, 0:-1, eps, 1],
            )
            axes[x].plot(m_plt_l, c_plt_l, "--", label="bad", lw=3)
            axes[x].plot(m_plt_h, c_plt_h, "-.", label="good", lw=3)
        axes[x].legend()
...
if __name__ == "__main__":
    ...
    P = np.array([(0.5, 0.5), (0.5, 0.5)])  # markov transition matrices
    ...
    sigma_psi_2mkv = np.sqrt(
        mean_preserving_spread(
            mean=lc.sigma_psi**2, probs=np.array([prob_l, prob_h]), l2mean_ratio=0.01
        )
    )

    sigma_eps_2mkv = np.sqrt(
        mean_preserving_spread(
            mean=lc.sigma_eps**2, probs=np.array([prob_l, prob_h]), l2mean_ratio=0.01
        )
    )
...
if __name__ == "__main__":
    ...
    print(
        "average permanent risk is "
        + str(av_sigma_psi)
        + " compared to objective model "
        + str(lc.sigma_psi)
    )
    print(
        "average transitory risk is "
        + str(av_sigma_eps)
        + " compared to objective model "
        + str(lc.sigma_eps)
    )
...
if __name__ == "__main__":
    ...
    for i, P in enumerate(P_ls):
        ...
        ## solve the model
        ms_star_sv, σs_star_sv = solve_model_backward_iter(lc_sv0, m_init_sv, σ_init_sv)
...
if __name__ == "__main__":
    ...
    for x, year in enumerate(years_left):
        ...
        i = lc.L - age
        m_plt_l, c_plt_l = (
            ms_stars_sv[0][i, 0:-1, eps_id, 0],
            σs_stars_sv[0][i, 0:-1, eps_id, 0],
        )
        m_plt_h, c_plt_h = (
            ms_stars_sv[0][i, 0:-1, eps_id, 1],
            σs_stars_sv[0][i, 0:-1, eps_id, 1],
        )

        axes[x].plot(
            m_plt_l,  ## 0 indicates the low risk state
            c_plt_l,
            "--",
            label="low risk",
            lw=3,
        ...
        axes[x].plot(
            m_plt_h,  ## 1 indicates the high risk state
            c_plt_h,
            "-.",
            label="high risk",
            lw=3,
...
if __name__ == "__main__":
    ...
    for x, year in enumerate(years_left):
        ...
        m_plt, c_plt = (
            ms_star_basic[i, 0:-1, eps_fix, 0],
            σs_star_basic[i, 0:-1, eps_fix, 0],
        )
        axes[x].plot(m_plt, c_plt, label="objective", lw=3)

        ## stochastic volatility
        m_plt_l, c_plt_l = (
            ms_stars_sv[0][i, 0:-1, eps_fix, 0],
            σs_stars_sv[0][i, 0:-1, eps_fix, 0],
        )
        m_plt_h, c_plt_h = (
            ms_stars_sv[0][i, 0:-1, eps_fix, 1],
            σs_stars_sv[0][i, 0:-1, eps_fix, 1],
        )
...
if __name__ == "__main__":
    ...
    P_uemkv = np.array([(0.2, 0.8), (0.2, 0.8)])  # markov transition matrices
...
if __name__ == "__main__":
    ...
    for i, P in enumerate(P_ls):
        ...
        ## solve the model
        ms_star_uemkv, σs_star_uemkv = solve_model_backward_iter(
            lc_uemkv, m_init_uemkv, σ_init_uemkv
        )
...
if __name__ == "__main__":
    ...
    for x, year in enumerate(years_left):
        ...
        i = lc_uemkv.L - age
        m_plt_u, c_plt_u = (
            ms_stars_uemkv[0][i, 0:-1, eps_id, 0],
            σs_stars_uemkv[0][i, 0:-1, eps_id, 0],
        )
        m_plt_e, c_plt_e = (
            ms_stars_uemkv[0][i, 0:-1, eps_id, 1],
            σs_stars_uemkv[0][i, 0:-1, eps_id, 1],
        )

        axes[x].plot(
            m_plt_u,  ## 0 indicates the low risk state
            c_plt_u,
            "--",
            label="unemployed",
            lw=3,
        ...
        axes[x].plot(
            m_plt_e,  ## 1 indicates the high risk state
            c_plt_e,
            "-.",
            label="employed",
            lw=3,
...
if __name__ == "__main__":
    ...
    sigma_psi_2mkv_cr = np.sqrt(
        mean_preserving_spread(
            mean=lc_uemkv.sigma_psi**2,
    ...
    sigma_eps_2mkv_cr = np.sqrt(
        mean_preserving_spread(
            mean=lc_uemkv.sigma_eps**2,
            probs=np.array([prob_l_cr, prob_h_cr]),
            l2mean_ratio=0.1,
        )
    )
...
if __name__ == "__main__":
    ...
    print(
        ...
        + str(av_sigma_psi_cr)
        + " compared to objective model "
        + str(lc_uemkv.sigma_psi)
    )
    print(
        "average transitory risk is "
        + str(av_sigma_eps_cr)
        + " compared to objective model "
        + str(lc_uemkv.sigma_eps)
    )
...
if __name__ == "__main__":
    ...
    for i, P in enumerate(P_ls):
        ...
        ## solve the model
        ms_star_cr, σs_star_cr = solve_model_backward_iter(lc_cr, m_init_cr, σ_init_cr)
...
if __name__ == "__main__":
    ...
    for x, year in enumerate(years_left):
        ...
        i = lc.L - age
        m_plt_l, c_plt_l = (
            ms_stars_cr[0][i, 0:-1, eps_id, 0],
            σs_stars_cr[0][i, 0:-1, eps_id, 0],
        )
        m_plt_h, c_plt_h = (
            ms_stars_cr[0][i, 0:-1, eps_id, 1],
            σs_stars_cr[0][i, 0:-1, eps_id, 1],
        )
...
if __name__ == "__main__":
    ...
    for x, year in enumerate(years_left):
        ...
        for eps in eps_ls:
            ## persistent
            m_plt_u, c_plt_u = (
                ms_stars_uemkv[0][i, 0:-1, eps, 0],
                σs_stars_uemkv[0][i, 0:-1, eps, 0],
            )
            m_plt_e, c_plt_e = (
                ms_stars_uemkv[0][i, 0:-1, eps, 1],
                σs_stars_uemkv[0][i, 0:-1, eps, 1],
            )
            axes[x].plot(m_plt_u, c_plt_u, "--", label="unemployed", lw=3)
            axes[x].plot(m_plt_e, c_plt_e, "-.", label="employed", lw=3)
            ## countercyclical
            m_plt_l, c_plt_l = (
                ms_stars_cr[0][i, 0:-1, eps, 0],
                σs_stars_cr[0][i, 0:-1, eps, 0],
            )
            m_plt_h, c_plt_h = (
                ms_stars_cr[0][i, 0:-1, eps, 1],
                σs_stars_cr[0][i, 0:-1, eps, 1],
            )
            axes[x].plot(
                m_plt_l,  ## 0 indicates the low risk state
                c_plt_l,
...
if __name__ == "__main__":
    ...
    for i, x in enumerate(x_ls):
        ...
        inf_liv.x = x
        m_inf_star, σ_inf_star = solve_model_iter(inf_liv, m_init, σ_init)
...
if __name__ == "__main__":
    ...
    for i, x in enumerate(x_ls):
        ## set different ma parameters
        inf_liv.x = x
        m_imp_star, σ_imp_star = solve_model_iter(imp_adjust, m_init, σ_init)
