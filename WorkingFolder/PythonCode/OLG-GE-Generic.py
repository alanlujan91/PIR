...
for x, year in enumerate(years_left):
    ...
    for k, model_name in enumerate(model_names):
        ...
        axes[x].plot(m_grid, c_func_e(m_grid), label=model_name + ",employed", lw=3)
    axes[x].legend()
...
HH.ComputeSSDist(ms_star=ms_stars[0], σs_star=σs_stars[0])
...
plt.spy(HH.initial_dist_u.reshape(n_m, -1), markersize=2)
plt.xlabel("p")
plt.ylabel("m")
...
plt.spy(HH.initial_dist_e.reshape(n_m, -1), markersize=2)
plt.xlabel("p")
plt.ylabel("m")
# -
...
ax.plot(
    share_agents_ap, share_ap, "r--", label="Model" + ", Gini={:.2f}".format(gini_this)
)
...
ax.plot(np.log(ap_grid_dist + 1e-5), ap_pdfs_dist)
...
ax.plot(age_lc[1:-1], np.log(A_life[:-1]), "r-o", label="model")
ax.set_xlabel("Age")
...
market_OLG_mkv = Market_OLG_mkv(households=HH, production=production)
...
plt.title("Life cycle profile of wealth")
ax.plot(
    age_lc[:-2], np.log(market_OLG_mkv.households.A_life)[:-1], "r-o", label="model"
)
# ax.set_ylim([-2.5,4])
...
ax.set_title("Wealth distribution")
ax.plot(
    np.log(market_OLG_mkv.households.ap_grid_dist + 1e-5),
