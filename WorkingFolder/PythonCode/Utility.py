...
# + code_folding=[0]
def policyfunc(lc, a_star, σ_star, discrete=True):
    """
...
# + code_folding=[0]
def policyfuncMA(lc, aϵ_star, σ_star):
    """
    * ifp is an instance of IFP
...
# + code_folding=[1]
@njit
def policyfuncMAjit(lc, aϵ_star, σ_star):
    """
    * ifp is an instance of IFP
       * aϵ_star is the endogenous grid solution
       * σ_star is optimal consumption on the grid
...
# + code_folding=[1]
## lorenz curve
def lorenz_curve(grid_distribution, pdfs, nb_share_grid=50):
    """
    parameters
    ======
...
# + code_folding=[1]
## lorenz curve
def wealth_share(grid_distribution, pdfs, top_agents_share=0.01):
    """
    parameters
    ======
    grid_distribution: grid on which distribution is defined
    ...
    where = min(
        [x for x in range(len(pdfs_cum)) if pdfs_cum[x] >= (1 - top_agents_share)]
    )
...
# + code_folding=[0]
def gini(agents_share, value_share):
    """
    input
    =====
...
def combine_ind_markov(Ps):
    ...
    assert len(P_combined) == n_combined, (
        "the dimension of the combined P should be " + str(n_combined)
    )
    return P_combined
...
def mean_preserving_spread(mean, probs, l2mean_ratio):
    """
...
class CDProduction:
    ...
    def __init__(
        self, Z=1.00, K=1.00, L=1.00, α=0.33, δ=0.025, target_KY=3.0, target_W=1.0
    ):
        self.Z = Z
        self.K = K
        self.L = L
    ...
    def normlize_Z(self, N_ss=1.0):
        ...
        Z_root, K_root = fsolve(distance, [0.7, 0.5])
...
@njit
def stationary_age_dist(L, n, LivPrb):
    """
...
def unemp_insurance2tax(μ, ue_fraction):
    """
    input
    =====
    μ: replcament ratio
...
def SS2tax(
    SS,  ## social security /pension replacement ratio
    T,  ## retirement years
    age_dist,  ## age distribution in the economy
    G,  ## permanent growth fractor lists over cycle
    emp_fraction,
):  ## fraction of employment in work age population
    pic_share = np.cumprod(G)  ## generational permanent income share
    pic_age_share = np.multiply(
        pic_share, age_dist
    )  ## generational permanent income share weighted by population weights

    dependence_ratio = np.sum(pic_age_share[T:]) / np.sum(
        pic_age_share[: T - 1] * emp_fraction
    )
...
# + code_folding=[1, 95]
@njit
def jump_to_grid(m_vals, perm_vals, probs, dist_mGrid, dist_pGrid):
    """
...
@njit
def jump_to_grid_fast(vals, probs, Grid):
    """
    ...
    for i in range(len(vals)):
        if mIndex[i] == -1:
            ...
            mlowerIndex = mIndex[i]
            mupperIndex = mIndex[i] + 1
            mlowerWeight = (Grid[mupperIndex] - vals[i]) / (
...
# + code_folding=[16]
@njit(parallel=True)
def gen_tran_matrix_fast(dist_mGrid, bNext, shk_prbs, perm_shks, tran_shks):
...
@njit(parallel=True)
def gen_tran_matrix(dist_mGrid, dist_pGrid, bNext, shk_prbs, perm_shks, tran_shks):
    TranMatrix = np.zeros(
    ...
    for i in prange(len(dist_mGrid)):
        for j in prange(len(dist_pGrid)):
            ...
            TranMatrix[:, i * len(dist_pGrid) + j] = jump_to_grid(
                mNext_ij, pNext_ij, shk_prbs, dist_mGrid, dist_pGrid
            )
    return TranMatrix
...
def average_heterogeneity(sigma_xi_psi, T, L, age_dist):
    """
    input
    ======
    ...
    sigma_xi_psi_cum2_all = np.concatenate((sigma_xi_psi_cum2, sigma_xi_psi_cum2_after))
