...
model_results = solve_models(
    models, ms_stars, σs_stars, model_name_list=model_names, ge=False
)

# + code_folding=[2] pycharm={"name": "#%%\n"}
## plot results from different models

line_patterns = ["g-v", "r-.", "b--", "cv", "k*", "g."]
...
for k, model in enumerate(models):
    ...
    scale_adjust = (
        np.log(np.array(SCF_profile["mv_lqwealth"])[3])
        - np.log(model_solution["A_life"])[0]
    )
    ax.plot(
        age_lc[1:],
...
ax.set_ylim([5, 12])
ax.vlines(lc_mkv.T + 25, 5, 12, color="k", label="retirement")
ax.bar(
    age_lc[1:],
