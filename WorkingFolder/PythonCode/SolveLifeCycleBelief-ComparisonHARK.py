...
# + code_folding=[]
inf_mkv_paras_dict = {
    "U": inf_paras["U"],  ## transitory ue risk
...
a_inf_star, σ_inf_star = solve_model_iter(inf_mkv, a_init, σ_init)
t_finish = time()
...
hark_mkv.PermGroFac = [np.array(2 * hark_mkv.PermGroFac)]
...
hark_mkv.IncShkDstn = [[unemployed_income_dist, employed_income_dist]]
...
for i, sigma_eps in enumerate(sigma_eps_ls):
    ...
    init_life_cycle_new["PermShkStd"] = [sigma_psi_ls[i]] * init_life_cycle_new[
        "T_cycle"
    ]
...
init_life_cycle_new["T_cycle"] = (
    lc_paras["L"] - 1
)  ## minus 1 because T_cycle is nb periods in a life cycle - 1 in HARK
init_life_cycle_new["CRRA"] = lc_paras["ρ"]
init_life_cycle_new["T_retire"] = lc_paras["T"] - 1
init_life_cycle_new["Rfree"] = lc_paras["R"]
init_life_cycle_new["LivPrb"] = [lc_paras["LivPrb"]] * init_life_cycle_new["T_cycle"]
init_life_cycle_new["PermGroFac"] = lc_paras["G"]
init_life_cycle_new["PermShkStd"] = [lc_paras["σ_ψ"]] * init_life_cycle_new[
    "T_retire"
] + [0.0] * years_retire
init_life_cycle_new["TranShkStd"] = [lc_paras["σ_θ"]] * init_life_cycle_new[
    "T_retire"
] + [0.0] * years_retire
...
init_life_cycle_new["UnempPrb"] = lc_paras[
    "U"
...
ax.plot(age_lc[1:], np.log(A_life), "r-", label="model from HARK")
