...
if calibrated_model == True:
    ...
    init_sigma_psi_av = average_heterogeneity(
        sigma_xi_psi, lc_paras["T"], lc_paras["L"], age_dist_ss
    )

    print(
        "Adjusting initial p dispersion by {} to account of heterogeneity in p ".format(
            init_sigma_psi_av
        )
    )
...
for x, year in enumerate(years_left):
    ...
    for k, model_name in enumerate(model_names):
        ...
        axes[x].plot(m_grid, c_func_u(m_grid), label=model_name + ",unemployed", lw=3)
        # axes[x].plot(m_grid,
        #             c_func_e(m_grid),
...
class HH_OLG_Markov:
    ...
    def __init__(self, model=None):
        ...
        self.age_dist = stationary_age_dist(
            model.L, n=lc_paras["n"], LivPrb=model.LivPrb
        )
...
def solve_1model(model, m_star, σ_star, n_m=n_m, n_p=n_p, model_name="model", ge=False):
    ...
    if ge:
        ...
        model_ge_dct = {
            "A": market_OLG_mkv_this.households.A,
        ...
        pickle.dump(
            model_ge_dct, open("./model_solutions/" + model_name + "_GE.pkl", "wb")
        )
...
model_results = solve_models(
    models, ms_stars, σs_stars, model_name_list=model_names, ge=True
)
...
ax.vlines(lc_mkv.T + 25, 6, 13, color="k", label="retirement")
...
ax.vlines(lc_mkv.T + 25, 6, 13, color="k", label="retirement")
ax.plot(age_lc, np.log(SCF_profile["mv_lqwealth"]), "k-", label="SCF (RHS)")
