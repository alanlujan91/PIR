...
LivProbs_y = np.array(
    life_table[(life_table["age"] > 25) & (life_table["age"] <= 25 + L)]["av_survial"]
)
...
if age_profile_data == "SIPP":
    ...
    lc_wages = np.array(
        age_profile[(age_profile["age"] >= 24) & (age_profile["age"] <= 24 + T)][
            "wage_age"
        ]
    )
...
if __name__ == "__main__":
    ...
    plt.plot(np.cumprod(lc_G_full), "k-v")
...
lc_G_sub = np.array(
    1 + SCE[(SCE["age"] >= 26) & (SCE["age"] < 64)].groupby("age")["rincexp"].mean()
)
## growth rates after retirement
...
sigma_eps_sub = np.sqrt(
    1 / (1 + kappa_sipp**2) * 0.04**2
)  ## 0.04 is an upper bound for the average PR in standard deviation from SCE
...
P_sub_q = np.array(
    [
        [SCE_est_q.loc["$q$"], 1 - SCE_est_q.loc["$q$"]],
        [1 - SCE_est_q.loc["$p$"], SCE_est_q.loc["$p$"]],
    ]
)
P_sub_y = np.array(
    [
        [SCE_est_y.loc["$q$"], 1 - SCE_est_y.loc["$q$"]],
...
life_cycle_paras_q = {
    "ρ": 2.0,
    "β": 0.97 ** (1 / 4),
    "P": np.array([[0.18, 0.82], [0.04, 0.96]]),
    "z_val": np.array([0.0, 1.0]),
    ...
    "P_sub": P_sub_q,
    "σ_ψ_2mkv": np.array(
        [SCE_est_q.loc["$\tilde\sigma^l_\psi$"], SCE_est_q.loc["$\tilde\sigma^h_\psi$"]]
    ),
    "σ_θ_2mkv": np.array(
        [
            SCE_est_q.loc["$\tilde\sigma^l_\theta$"],
...
if "$\tilde \mho^l$" in SCE_est_q.index:
    life_cycle_paras_q["mho_2mkv"] = np.array(
        [SCE_est_q.loc["$\tilde \mho^l$"], SCE_est_q.loc["$\tilde \mho^h$"]]
    )
    life_cycle_paras_q["E_2mkv"] = np.array(
        [SCE_est_q.loc["$\tilde E^l$"], SCE_est_q.loc["$\tilde E^h$"]]
    )

else:
    life_cycle_paras_q["mho_2mkv"] = np.array(
        [life_cycle_paras_q["P"][0, 0], life_cycle_paras_q["P"][0, 0]]
    )
    life_cycle_paras_q["E_2mkv"] = np.array(
        [life_cycle_paras_q["P"][1, 1], life_cycle_paras_q["P"][1, 1]]
    )
# -
...
life_cycle_paras_y = {
    ...
    "P_sub": P_sub_y,
    "σ_ψ_2mkv": np.array(
        [SCE_est_y.loc["$\tilde\sigma^l_\psi$"], SCE_est_y.loc["$\tilde\sigma^h_\psi$"]]
    ),
    "σ_θ_2mkv": np.array(
        [
            SCE_est_y.loc["$\tilde\sigma^l_\theta$"],
...
if "$\tilde \mho^l$" in SCE_est_y.index:
    life_cycle_paras_y["mho_2mkv"] = np.array(
        [SCE_est_y.loc["$\tilde \mho^l$"], SCE_est_y.loc["$\tilde \mho^h$"]]
    )
    life_cycle_paras_y["E_2mkv"] = np.array(
        [SCE_est_y.loc["$\tilde E^l$"], SCE_est_y.loc["$\tilde E^h$"]]
    )

else:
    life_cycle_paras_y["mho_2mkv"] = np.array(
        [life_cycle_paras_y["P"][0, 0], life_cycle_paras_y["P"][0, 0]]
    )
    life_cycle_paras_y["E_2mkv"] = np.array(
        [life_cycle_paras_y["P"][1, 1], life_cycle_paras_y["P"][1, 1]]
    )
# -
...
block_all = [risk, initial, lifecycle, prefernece, policy, production, subjective]
...
for i, block in enumerate(block_all):
    model_paras_by_block[blocknames[i]] = {
        k: v for k, v in model_paras.items() if k in block
    }
# -
...
# +
model_paras_by_block_df = pd.DataFrame.from_dict(
    {
        (i, j): model_paras_by_block[i][j]
...
Mindex = pd.MultiIndex.from_tuples(
    list(model_paras_by_block_df.index), names=["block", "parameter"]
)
...
model_paras_by_block_df.loc[
    "initial condition", "source"
] = "estimated for age 25 in the 2016 SCF"
model_paras_by_block_df.loc[
    ("initial condition", "bequest_ratio"), "source"
] = "assumption"
...
model_paras_by_block_df.loc[
    ("preference", "β"), "source"
] = "targeting average wealth/income ratio"
...
model_paras_by_block_df.loc[
    ("production", "W"), "source"
] = "target values in steady state"
model_paras_by_block_df.loc[
    ("production", "K2Y ratio"), "source"
] = "target values in steady state"
...
model_paras_by_block_df = model_paras_by_block_df[
