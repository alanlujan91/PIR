...
mean_one_log_normal_draws = np.exp(
    np.random.randn(100000) * sigma_fake - sigma_fake**2 / 2
)
...
mu_fake_est, sigma_fake_est = (
    np.log(scale),
    shape,
)  # mu, sigma ## mu=-sigma^2/2 for mean-one log normal
...
# + {"code_folding": [0]}
def Est_PR_log_normal_simple(PRs, est_PR, transform=True):
    """
    input
    =====
    ...
    if transform == False:
        shape, loc, scale = stats.lognorm.fit(PRs, floc=0)
        mu_PR_est, sigma_PR_est = (
            np.log(scale),
            shape,
        )  # mu, sigma ## mu=-sigma^2/2 for mean-one log normal
...
mu_PR_est_SCE, sigma_PR_est_SCE, sigma_xi_est_SCE = Est_PR_log_normal_simple(
    PR_ind_av, est_PR_Low, transform=True
)
...
PRs_grid = np.linspace(np.min(PR_ind_av), np.max(PR_ind_av), 100)
...
mu_PRn_est_SCE, sigma_PRn_est_SCE, sigma_xi_n_est_SCE = Est_PR_log_normal_simple(
    PRn_ind_av, est_PR_Low, transform=True
)
print("Estimated mean nominal PR", str(mu_PRn_est_SCE))
print("Estimated standard deviation in nominal PR", str(sigma_PRn_est_SCE))
print(
    "Estimated nominal wedge due to unobserved heterogeneity", str(sigma_xi_n_est_SCE)
)

PRns_grid = np.linspace(np.min(PRn_ind_av), np.max(PRn_ind_av), 100)
...
plt.axvline(np.sqrt(est_PR_Low), linestyle="--", color="blue", label="Estimated PR")
...
U2U_trans_mu_est, sigma_trans_U2U_est, loc_trans_U2U_est = (
    np.log(scale_U2U),
    shape_U2U,
    loc_U2U,
)

U2U_est_dist = stats.lognorm(s=shape_U2U, loc=loc_U2U, scale=scale_U2U)

U2U_grid = np.linspace(np.min(U2U_ind_av), np.max(U2U_ind_av), 100)
...
E2E_trans_mu_est, sigma_trans_E2E_est, loc_trans_E2E_est = (
    np.log(scale_E2E),
    shape_E2E,
    loc_E2E,
)
...
E2E_draws = stats.lognorm.rvs(s=shape_E2E, loc=loc_E2E, scale=scale_E2E, size=10000)
plt.figure(figsize=(6, 4))
...
plt.axvline(
    av_Exp_SCE, color="black", label="Average Exp={:.1f}".format(round(av_Exp_SCE))
)
...
Exps_sim_simple = stats.lognorm.rvs(
    s=shape_Exp, loc=loc_Exp, scale=scale_Exp, size=1000
)
exp_mu_est, sigma_exp_est, loc_exp_est = (
    np.log(scale_Exp),
    shape_Exp,
    loc_Exp,
)  # mu, sigma ## mu=-sigma^2/2 for mean-one log normal
