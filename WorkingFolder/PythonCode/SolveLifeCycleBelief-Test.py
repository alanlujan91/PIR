...
if __name__ == "__main__":
    ...
    plt.plot(m_init[:, 0, 0, 0], σ_init[:, 0, 0, 0])
# -
...
if __name__ == "__main__":
    ...
    ## solve backward
    ms_star_no_ret, σs_star_no_ret = solve_model_backward_iter(
        lc_no_ret, m_init, σ_init
    )
...
if __name__ == "__main__":
    ...
    for x, year in enumerate(years_left):
        ...
        axes[x].plot(m_plt, c_plt, label="No retirement", lw=3)
        axes[x].legend()
...
if __name__ == "__main__":
    ...
    ## solve backward
    ms_star_a_max, σs_star_a_max = solve_model_backward_iter(lc_a_max, m_init, σ_init)
...
if __name__ == "__main__":
    ...
    ## solve backward
    ms_star_2t, σs_star_2t = solve_model_backward_iter(lc_2t, m_init, σ_init)
...
if __name__ == "__main__":
    ...
    ## solve backward
    ms_star_no_trisk, σs_star_no_trisk = solve_model_backward_iter(
        lc_no_trisk, m_init, σ_init
    )
...
if __name__ == "__main__":
    ...
    for x, year in enumerate(years_left):
        ...
        axes[x].plot(m_plt, c_plt, label="No t risks", lw=3)
        axes[x].legend()
        axes[x].set_xlim((0.0, np.max(m_plt[-2])))
        axes[x].set_ylim((0.0, np.max(c_plt[-2])))
        axes[x].set_xlabel("asset")
...
if __name__ == "__main__":
    ...
    ## solve backward
    ms_star_no_prisk, σs_star_no_prisk = solve_model_backward_iter(
        lc_no_prisk, m_init, σ_init
    )
...
if __name__ == "__main__":
    ...
    for x, year in enumerate(years_left):
        ...
        axes[x].plot(m_plt, c_plt, label="No p risks", lw=3)
        axes[x].legend()
        axes[x].set_xlim((0.0, np.max(m_plt[-2])))
        axes[x].set_ylim((0.0, np.max(c_plt[-2])))
        axes[x].set_xlabel("asset")
...
# + code_folding=[]
sub_minus_obj = compare_2solutions(ms_stars, σs_stars)

plt.hist(sub_minus_obj.flatten(), bins=50)
...
if __name__ == "__main__":
    ...
    for x, year in enumerate(years_left):
        ...
        for k, model_name in enumerate(model_names):
            ...
            axes[x].plot(m_grid, c_func(m_grid), label=model_name, lw=3)
            axes[x].set_xlim((0.0, np.max(m_plt[-2])))
            axes[x].set_ylim((0.0, np.max(c_plt[-2])))
        axes[x].legend()
...
if __name__ == "__main__":
    ...
    for i, sigma_psi in enumerate(sigma_psi_ls):
        ...
        ## solve backward
        ms_star, σs_star = solve_model_backward_iter(lc_baseline, m_init, σ_init)
        ms_stars.append(ms_star)
        σs_stars.append(σs_star)
...
if __name__ == "__main__":
    ...
    for x, year in enumerate(years_left):
        ...
        for k, model_name in enumerate(model_names):
            ...
            # c_func = lambda m: interp(m_plt,c_plt,m)
            axes[x].plot(m_plt, c_plt, label=model_name, lw=3)
...
if __name__ == "__main__":
    ...
    lc_trans_mkv_paras["P"] = np.array([[U_prb, 1 - U_prb], [U_prb, 1 - U_prb]])
    ...
    model_names = ["transitory UE", "markov UE:special case"]
...
trans_mkv_diff = compare_2solutions(ms_stars, σs_stars)

plt.hist(trans_mkv_diff.flatten(), bins=50)
