    ...
    def terminal_solution(self):
        ...
        if self.q == 0.0:
            ...
            for z in range(n_z):
                for f in range(n_f):
                    for j in range(k2):
                        σ_init[:, j, z, f] = (self.q * self.a_grid ** (-self.ρ_b)) ** (
                            -1 / self.ρ
                        )
...
def EGM_combine(
    ...
    ########################################################
    σ = lambda m, ϵ, z, f: mlinterp(
        (mϵ_in[:, 0, z, f], eps_grid), σ_in[:, :, z, f], (m, ϵ)
    )
                ...
                for f in range(n_f):
                    ...
                    for z_hat in range(n_z):
                        ...
                        if lc.state_dependent_risk == False:
                            ...
                            P = np.array(
                                [
                                    [U2U_2mkv[z_hat], 1 - U2U_2mkv[z_hat]],
                ...
                if (
                    ...
                    if age <= lc.T - 1:
                        ...
                        if state_dependent_belief:
                            self_min_a = (
                                -np.exp(np.min(eps_shk_mkv_draws[f, :])) * G / R
                            )
                        else:
                            self_min_a = -np.exp(np.min(eps_shk_draws)) * G / R
                        if state_dependent_risk:
                            self_min_a = (
                                -np.exp(np.min(eps_shk_mkv_draws[z, :])) * G / R
                            )
                        else:
                            self_min_a = -np.exp(np.min(eps_shk_draws)) * G / R
...
def compare_2solutions(ms_stars, σs_stars):
    ...
    for i in range(n_age):
        for j in range(n_eps):
            for k in range(n_z):
                for f in range(n_f):
                    c_func1 = lambda m: interp(
                        m_star1[i, :, j, k, f], σ_star1[i, :, j, k, f], m
                    )
                    c_stars1[i, :, j, k, f] = c_func1(m_grid)
                    c_func2 = lambda m: interp(
                        m_star2[i, :, j, k, f], σ_star2[i, :, j, k, f], m
                    )
...
if __name__ == "__main__":
    ...
    plt.plot(m_init[0:-1, 0, 0, 0], σ_init[0:-1, 0, 0, 0])
...
if __name__ == "__main__":
    ...
    ax.plot(m_vec[0:-1, 0, 0, 0], σ_vec[0:-1, 0, 0, 0], label="T")
    ax.set_xlabel("m: cash in hand")
    ax.set_ylabel("c: consumption")
    ## solve backward

    for it in range(100):
        m_next, σ_next = EGM_combine(m_vec, σ_vec, 10, lc_basic)
        if it < 5:
            ax.plot(
                m_next[0:-1, 0, 0, 0], σ_next[0:-1, 0, 0, 0], label="T-" + str(it + 1)
            )
...
if __name__ == "__main__":
    ...
    ## solve backward
    ms_star_basic, σs_star_basic = solve_model_backward_iter(lc_basic, m_init, σ_init)
...
if __name__ == "__main__":
    ...
    P_sub = np.array([[0.95, 0.05], [0.1, 0.9]])
...
if __name__ == "__main__":
    ...
    ## solve the model
    ms_star_sub, σs_star_sub = solve_model_backward_iter(
        lc_uemkv_sub, m_init_sub, σ_init_sub
    )
...
if __name__ == "__main__":
    ...
    for x, year in enumerate(years_left):
        ...
        i = lc_basic.L - age
        m_plt_u_l, c_plt_u_l = (
            ms_star_sub[i, 0:-1, eps_id, 0, 0],
            σs_star_sub[i, 0:-1, eps_id, 0, 0],
        )
        m_plt_u_h, c_plt_u_h = (
            ms_star_sub[i, 0:-1, eps_id, 0, 1],
            σs_star_sub[i, 0:-1, eps_id, 0, 1],
        )

        m_plt_e_l, c_plt_e_l = (
            ms_star_sub[i, 0:-1, eps_id, 1, 0],
            σs_star_sub[i, 0:-1, eps_id, 1, 0],
        )
        m_plt_e_h, c_plt_e_h = (
            ms_star_sub[i, 0:-1, eps_id, 1, 1],
            σs_star_sub[i, 0:-1, eps_id, 1, 1],
        )

        axes[x].plot(
            m_plt_u_l,  ## 0 indicates the low risk state
            c_plt_u_l,
            "--",
        ...
        axes[x].plot(
            m_plt_u_h,  ## 1 indicates the high risk state
            c_plt_u_h,
            "-.",
...
if __name__ == "__main__":
    ...
    plt.plot(
        ms_inf_star[0:-1, eps_fix, 0, 0],
        σs_inf_star[0:-1, eps_fix, 0, 0],
        label="uemp: low risk belief",
        lw=3,
    )
    plt.plot(
        ms_inf_star[0:-1, eps_fix, 0, 1],
