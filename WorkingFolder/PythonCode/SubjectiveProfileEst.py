...
from MarkovSwitchingEst import (
    prob_func_inv,
    exp_func_inv,
)
...
vars_job = [
    ...
    "Q10_2",  # part-time
    "Q12new",
]  ## =1 worked for others; = 2 self-employment
...
vars_all_reg_long = (
    vars_id + moms_nom + moms_real + ue_risks + vars_demog + vars_demog_sub + vars_job
)
...
SCEM["E2E_prob"] = 1 - SCEM["UE_s"] * (
    1 - SCEM["UE_f"]
)  ## 1- prob(loses the job and not finding one)
...
print(
    "nb of observations after dropping low numeracy/low education sample:",
    str(len(SCEM)),
)
...
vars_list = ["lrincvar", "erincexp", "U2U_prob_e", "E2E_prob_e"]
var_name = ["wage risk", "expected wage growth", "U2U", "E2E"]
...
vars_rd_list = ["lrincvar", "erincexp", "U2U_prob_e", "E2E_prob_e"]
# -
...
SCE_list = [
    SCEM_sub[SCEM_sub["userid"] == ID][vars_rd_list].T.to_numpy() for ID in unique_ids
]
...
fig, axes = plt.subplots(len(vars_rd_list), 1, figsize=(10, 8))
axes[0].set_title("A random respondent's risk perceptions from the survey")
axes[0].plot(SCE_list[random_id][0, :], "o-", label="perceived earning risk")
axes[0].legend(loc=1)

axes[1].plot(SCE_list[random_id][1, :], "o-", label="expected earning growth factor")
axes[1].legend(loc=1)

axes[2].plot(SCE_list[random_id][2, :], "o-", label="perceived U2E prob")
axes[2].legend(loc=1)

axes[3].plot(SCE_list[random_id][3, :], "o-", label="perceived E2E prob")
axes[3].legend(loc=1)
...
if AR == 0:
    if nb_var == 3:
        paras_init = np.array(
            [0.1, 0.1, 0.1, 0.7, 0.7, 0.1, 0.1, 0.1, 0.7, 0.7, 0.1, -0.1, 0.1, 0.7, 0.7]
        )
        guess = np.array(
            [0.2, 0.4, -0.5, 0.1, 0.4, 0.2, 0.4, -2, 0.1, 0.4, 0.2, -0.1, -2, 0.1, 0.4]
        )
...
## model instance
SCE_mkv2 = Markov2Switching(AR=AR, paras=paras_init, nb_var=nb_var)


## objective func
SCE_obj = lambda para: -SCE_mkv2.log_likelihood(SCE_list, para)[
    0
...
model_para_q_est["q"], model_para_q_est["p"] = mkv2_M2Q(
    SCE_para_model_est["q"], SCE_para_model_est["p"]
)
...
model_para_q_est["\tilde\sigma_\psi^l"] = np.sqrt(
    3 * np.exp(alpha) / (12 + 1 / (12 * kappa**2))
)
model_para_q_est["\tilde\sigma_\theta^l"] = (
    1 / 3 * model_para_q_est["\tilde\sigma_\psi^l"] / kappa
)
model_para_q_est["\tilde\sigma_\psi^h"] = np.sqrt(
    3 * np.exp(alpha + beta) / (12 + 1 / (12 * kappa**2))
)
model_para_q_est["\tilde\sigma_\theta^h"] = (
    1 / 3 * model_para_q_est["\tilde\sigma_\psi^h"] / kappa
)
...
if nb_var == 3:
    ...
    model_para_q_est["\tilde \mho^l"], model_para_q_est["\tilde E^l"] = mkv2_Y2Q(
        SCE_mkv2.prob_func(alpha_q), SCE_mkv2.prob_func(alpha_p)
    )

    model_para_q_est["\tilde \mho^h"], model_para_q_est["\tilde E^h"] = mkv2_Y2Q(
        SCE_mkv2.prob_func(alpha_q + beta_q), SCE_mkv2.prob_func(alpha_p + beta_p)
    )
...
SCE_para_est_q_df = pd.DataFrame.from_dict(
    model_para_q_est, orient="index", dtype=None, columns=["baseline"]
)
...
model_para_y_est["q"], model_para_y_est["p"] = mkv2_M2Q(
    SCE_para_model_est["q"], SCE_para_model_est["p"]
)
...
model_para_y_est["\tilde\sigma_\psi^l"] = np.sqrt(
    12 * np.exp(alpha) / (12 + 1 / (12 * kappa**2))
)
model_para_y_est["\tilde\sigma_\theta^l"] = (
    1 / 12 * model_para_q_est["\tilde\sigma_\psi^l"] / kappa
)
model_para_y_est["\tilde\sigma_\psi^h"] = np.sqrt(
    12 * np.exp(alpha + beta) / (12 + 1 / (12 * kappa**2))
)
model_para_y_est["\tilde\sigma_\theta^h"] = (
    1 / 12 * model_para_q_est["\tilde\sigma_\psi^h"] / kappa
)
...
if nb_var > 1:
    ...
    (
        model_para_y_est["\tilde \mho^l"],
        model_para_y_est["\tilde E^l"],
    ) = SCE_mkv2.prob_func(alpha_q), SCE_mkv2.prob_func(alpha_p)

    (
        model_para_y_est["\tilde \mho^h"],
        model_para_y_est["\tilde E^h"],
    ) = SCE_mkv2.prob_func(alpha_q + beta_q), SCE_mkv2.prob_func(alpha_p + beta_p)
...
SCE_para_est_y_df = pd.DataFrame.from_dict(
    model_para_y_est, orient="index", dtype=None, columns=["baseline"]
)
