...
# + code_folding=[]
from SolveLifeCycleBelief import (
    LifeCycle,
    solve_model_backward_iter,
    compare_2solutions,
)
...
lc_paras_y["P_sub"] = np.array(
    [[0.7, 0.3], [0.1, 0.9]]
)  ## a very transitory low risk state and a persistent high risk state
...
ojb_minus_sub = compare_2solutions(ms_stars[0:2], σs_stars[0:2])

plt.hist(ojb_minus_sub.flatten(), bins=50)
...
for x, year in enumerate(years_left):
    ...
    for k, model_name in enumerate(model_names):
        if k <= 3:
            m_plt_u_l, c_plt_u_l = (
                ms_stars[k][i, 0:-1, 0, z_l, 0],
                σs_stars[k][i, 0:-1, 0, z_l, 0],
            )
            m_plt_u_h, c_plt_u_h = (
                ms_stars[k][i, 0:-1, 0, z_l, 1],
                σs_stars[k][i, 0:-1, 0, z_l, 1],
            )
            m_plt_e_l, c_plt_e_l = (
                ms_stars[k][i, 0:-1, 0, z_h, 0],
                σs_stars[k][i, 0:-1, 0, z_h, 0],
            )
            m_plt_e_h, c_plt_e_h = (
                ms_stars[k][i, 0:-1, 0, z_h, 1],
                σs_stars[k][i, 0:-1, 0, z_h, 1],
            )
            # c_func_u = lambda m: interp(m_plt_u,c_plt_u,m)
            # c_func_e = lambda m: interp(m_plt_e,c_plt_e,m)
            axes[x].plot(m_plt_u_l, c_plt_u_l, label=model_name + ",UMP", lw=3)
            axes[x].plot(m_plt_e_l, c_plt_e_l, label=model_name + ",EMP", lw=3)
...
def calc_transition_matrix(
    ...
    perm_shks_mkv0 = np.exp(
        np.repeat(model.psi_shk_mkv_draws[0, :], len(model.psi_shk_true_draws))
    )

    perm_shks_mkv1 = np.exp(
        np.repeat(model.psi_shk_mkv_draws[1, :], len(model.psi_shk_true_draws))
    )

    tran_shks_mkv0 = np.exp(
        np.repeat(model.eps_shk_mkv_draws[0, :], len(model.psi_shk_true_draws))
    )

    tran_shks_mkv1 = np.exp(
        np.repeat(model.eps_shk_mkv_draws[1, :], len(model.psi_shk_true_draws))
    )
    ...
    for k in range(model.L):  ## loop over agents at different ages, k
        ...
        for m_id, m in enumerate(this_dist_mGrid):
            ...
            this_Cnow_u_f0 = mlinterp(
                (ms_star[year_left, :, 0, 0, 0], model.eps_grid),
                σs_star[year_left, :, :, 0, 0],
                (m, fix_epsGrid),
            )
            Cnow_u_f0[m_id] = this_Cnow_u_f0

            this_Cnow_u_f1 = mlinterp(
                (ms_star[year_left, :, 0, 0, 1], model.eps_grid),
                σs_star[year_left, :, :, 0, 1],
                (m, fix_epsGrid),
            )
            Cnow_u_f1[m_id] = this_Cnow_u_f1

            this_Cnow_e_f0 = mlinterp(
                (ms_star[year_left, :, 0, 0, 0], model.eps_grid),
                σs_star[year_left, :, :, 0, 0],
                (m, fix_epsGrid),
            )
            ...
            this_Cnow_e_f1 = mlinterp(
                (ms_star[year_left, :, 0, 1, 1], model.eps_grid),
        ...
        if fast == True:
            ...
            TranMatrix_u_f0_u = gen_tran_matrix_fast(
                this_dist_mGrid,
                bNext_u_f0,
                shk_prbs_ntrl0,
                perm_shks_G_ef0,
            ...
            TranMatrix_u_f1_u = gen_tran_matrix_fast(
                this_dist_mGrid,
                bNext_u_f1,
                shk_prbs_ntrl1,
                perm_shks_G_ef1,
            ...
            TranMatrix_u_f0_e = gen_tran_matrix_fast(
                this_dist_mGrid,
                bNext_u_f0,
                shk_prbs_ntrl0,
                perm_shks_G_ef0,
            ...
            TranMatrix_u_f1_e = gen_tran_matrix_fast(
                this_dist_mGrid,
                bNext_u_f1,
                shk_prbs_ntrl1,
                perm_shks_G_ef1,
            ...
            TranMatrix_e_f0_e = gen_tran_matrix_fast(
                this_dist_mGrid,
                bNext_e_f0,
                shk_prbs_ntrl0,
                perm_shks_G_ef0,
            ...
            TranMatrix_e_f1_e = gen_tran_matrix_fast(
                this_dist_mGrid,
                bNext_e_f1,
                shk_prbs_ntrl1,
                perm_shks_G_ef1,
            ...
            TranMatrix_e_f0_u = gen_tran_matrix_fast(
                this_dist_mGrid,
                bNext_e_f0,
                shk_prbs_ntrl0,
                perm_shks_G_ef0,
            ...
            TranMatrix_e_f1_u = gen_tran_matrix_fast(
                this_dist_mGrid,
                bNext_e_f1,
                shk_prbs_ntrl1,
                perm_shks_G_ef1,
            ...
            TranMatrix_u_f0_u = gen_tran_matrix(
                this_dist_mGrid,
                this_dist_pGrid,
                bNext_u_f0,
                shk_prbs,
            ...
            TranMatrix_u_f1_u = gen_tran_matrix(
                this_dist_mGrid,
                this_dist_pGrid,
                bNext_u_f1,
                shk_prbs,
            ...
            TranMatrix_u_f0_e = gen_tran_matrix(
                this_dist_mGrid,
                this_dist_pGrid,
                bNext_u_f0,
                shk_prbs,
            ...
            TranMatrix_u_f1_e = gen_tran_matrix(
                this_dist_mGrid,
                this_dist_pGrid,
                bNext_u_f1,
                shk_prbs,
            ...
            # Generate Transition Matrix for e2u for belief state 0
            TranMatrix_e_f0_u = gen_tran_matrix(
                this_dist_mGrid,
                this_dist_pGrid,
                bNext_e_f0,
                shk_prbs,
            ...
            # Generate Transition Matrix for e2u for belief state 1
            TranMatrix_e_f1_u = gen_tran_matrix(
                this_dist_mGrid,
                this_dist_pGrid,
                bNext_e_f1,
                shk_prbs,
            ...
            # Generate Transition Matrix for e2e for belief state 0
            TranMatrix_e_f0_e = gen_tran_matrix(
                this_dist_mGrid,
                this_dist_pGrid,
                bNext_e_f0,
                shk_prbs,
            ...
            # Generate Transition Matrix for e2e for belief state 1
            TranMatrix_e_f1_e = gen_tran_matrix(
                this_dist_mGrid,
                this_dist_pGrid,
                bNext_e_f1,
                shk_prbs,
        ...
        tran_matrix_u_f0 = (
            markov_array2[0, 1] * TranMatrix_u_f0_e
            + markov_array2[0, 0] * TranMatrix_u_f0_u
        )  # This is the transition for someone who's state today is unemployed
        tran_matrix_e_f0 = (
            markov_array2[1, 1] * TranMatrix_e_f0_e
            + markov_array2[1, 0] * TranMatrix_e_f0_u
        )  # This is the transition for someone who's state is employed today
        tran_matrix_u_f1 = (
            markov_array2[0, 1] * TranMatrix_u_f1_e
            + markov_array2[0, 0] * TranMatrix_u_f1_u
        )
        tran_matrix_e_f1 = (
            markov_array2[1, 1] * TranMatrix_e_f1_e
            + markov_array2[1, 0] * TranMatrix_e_f1_u
        )

        ## merge to the life cycle list
        tran_matrix_u_f0_list.append(
            tran_matrix_u_f0
        )  # This is the transition for someone who's state today is unemployed
    ...
    tran_matrix_list = List(
        [
            tran_matrix_u_f0_list,
...
def initial_distribution_e(
    ...
    NewBornDist = jump_to_grid(
        ...
        + init_b / init_p_draws
        + model.transfer,  ## initial transitory risks and accidental bequest transfer
        init_p_draws,
        shk_prbs,
        dist_mGrid,
        dist_pGrid,
...
def flatten_dist(
    grid_lists,  ## (nb.z x nb.f) x L x nb x nm x np
...
class HH_OLG_Markov:
    ...
    def __init__(self, model=None):
        ...
        self.ss_dstn_combined = combine_ind_markov(
            [cal_ss_2markov(model.P), cal_ss_2markov(model.P_sub)]
        )
    ...
    def ComputeSSDist(self, ms_star=None, σs_star=None):
        ...
        ap_u_f1_PolGrid_list = [
            np.multiply.outer(a_PolGrid_list[2][k], p_dist_grid_list[k]).flatten()
            for k in range(model.L)
        ]

        ## c and a for e for belief 1 (index 3)
        ap_e_f1_PolGrid_list = [
            np.multiply.outer(a_PolGrid_list[3][k], p_dist_grid_list[k]).flatten()
            for k in range(model.L)
        ]
        ...
        ap_ratio_u_f1_PolGrid_list = [
            np.repeat(a_PolGrid_list[2][k], len(p_dist_grid_list[k]))
            for k in range(model.L)
        ]

        ## c and a for e for belief 1 (index 3)
        ap_ratio_e_f1_PolGrid_list = [
            np.repeat(a_PolGrid_list[3][k], len(p_dist_grid_list[k]))
            for k in range(model.L)
        ]

        # a policy grid

        ap_ratio_PolGrid_list = [
            ap_ratio_u_f0_PolGrid_list,
            ap_ratio_e_f0_PolGrid_list,
            ap_ratio_u_f1_PolGrid_list,
            ap_ratio_e_f1_PolGrid_list,
        ...
        self.ap_grid_dist, self.ap_pdfs_dist = flatten_dist(
            ap_PolGrid_list, dist_lists, ss_dstn_combined, age_dist
        )

        ## stroe wealth to permanent income ratio distribution

        self.a_grid_dist, self.a_pdfs_dist = flatten_dist(
            ap_ratio_PolGrid_list, dist_lists, ss_dstn_combined, age_dist
        )
    ...
    def Aggregate(self):
        ...
        self.A = AggregateDist(ap_PolGrid_list, dist_lists, ss_dstn_combined, age_dist)
    ...
    def AggregatebyAge(self):
        ...
        for t in range(model.L):
            ...
            A_this_age = AggregateDist(
                ap_PolGrid_list, dist_lists, ss_dstn_combined, age_dist_sparse
            )
    ...
    def get_lifecycle_dist(self):
        ...
        for t in range(model.L):
            ...
            ap_grid_dist_this_age, ap_pdfs_dist_this_age = flatten_dist(
                ap_PolGrid_list, dist_lists, ss_dstn_combined, age_dist_sparse
            )
...
class Market_OLG_mkv:
    ...
    def __init__(self, households=None, production=None):
        ...
        self.households.uemp_ss = (
            np.sum(age_dist[: T - 1]) * ss_dstn[0]
        )  ## unemployed pop fraction
    ...
    def StE_K_d(
        ...
        model.λ = unemp_insurance2tax(model.unemp_insurance, uemp_now)
        print("Tax rate", str(model.λ))

        ## obtain social security rate balancing the SS replacement ratio

        model.λ_SS = SS2tax(
            model.pension,  ## social security /pension replacement ratio
        ...
        n_m = 40
        n_p = 20
        households.define_distribution_grid(num_pointsM=n_m, num_pointsP=n_p)
        households.ComputeSSDist(ms_star=ms_star, σs_star=σs_star)
    ...
    def get_equilibrium_k(self):
        ...
        K_eq = op.fixed_point(eq_func, x0=6.0)
    ...
    def get_equilibrium_dist(self):
        ...
        ## obtain tax rate from the government budget balance
        model.λ = unemp_insurance2tax(model.unemp_insurance, households.uemp_ss)
        ...
        households.define_distribution_grid(num_pointsM=40, num_pointsP=50)
        households.ComputeSSDist(ms_star=ms_star, σs_star=σs_star)
...
def solve_1model(model, m_star, σ_star, n_m=n_m, n_p=n_p, model_name="model", ge=False):
    ...
    if ge:
        ...
        market_OLG_mkv_this = Market_OLG_mkv(households=HH_this, production=production)
        ...
        ## lorenz
        (
            share_agents_ap_ge_this,
            share_ap_ge_this,
        ) = market_OLG_mkv_this.households.Lorenz(variable="a")

        ## gini in ge

        gini_this_ge = gini(share_agents_ap_ge_this, share_ap_ge_this)

        ## distribution
        zero_wealth_id_ge = np.where(market_OLG_mkv_this.households.a_grid_dist <= 1.0)
        h2m_share_ge = market_OLG_mkv_this.households.a_pdfs_dist[
            zero_wealth_id_ge
        ].sum()
        ...
        ## save it as a pkl
        pickle.dump(
            model_dct_ge, open("./model_solutions/" + model_name + "_GE.pkl", "wb")
        )


def solve_models(model_list, ms_star_list, σs_star_list, model_name_list, ge=False):
...
solve_models(models, ms_stars, σs_stars, model_name_list=model_names, ge=True)

# + code_folding=[0, 11, 37, 70]
## plot results from different models

line_patterns = ["g-v", "r-.", "b--", "y."]

## Lorenz curve of steady state wealth distribution

fig, ax = plt.subplots(figsize=(8, 6))
for k, model in enumerate(models):
    model_solution = pickle.load(
        open("./model_solutions/" + model_names[k] + "_PE.pkl", "rb")
    )
    ax.plot(
...
ax.plot(SCF_share_agents_ap, SCF_share_ap, "k-.", label="SCF")
ax.plot(
    model_solution["share_agents_ap"],
    model_solution["share_agents_ap"],
    "k-",
...
for k, model in enumerate(models):
    ...
    scale_adjust = np.log(np.array(SCF_profile["mv_lqwealth"])[2]) - np.log(
        model_solution["A_life"][0]
    )
...
ax.vlines(lc_mkv.T + 25, 8, 12, color="k", label="retirement")
ax.plot(age_lc, np.log(SCF_profile["mv_lqwealth"]), "k-", label="SCF liquid wealth")
ax.set_xlabel("Age")
...
for k, model in enumerate(models):
    ...
    ax.plot(
        np.log(model_solution["ap_grid_dist"] + 1e-5),
...
for k, model in enumerate(models):
    model_solution = pickle.load(
        open("./model_solutions/" + model_names[k] + "_GE.pkl", "rb")
    )
    ax.plot(
        model_solution["share_agents_ap"],
        model_solution["share_ap"],
