...
def unzip_survey_file(year="2013"):
    ...
    if int(year) < 1989:
        url = (
            "http://www.federalreserve.gov/econres/files/"
            + year
            + "_scf"
            + year[2:]
            + "bs.zip"
...
model = smf.ols(formula="lnorminc~ age+age2+age3+age4", data=df2016)
results = model.fit()
...
data_plot = df2016[["norminc", "wgt"]][
    df2016["norminc"] < df2016["norminc"].quantile(0.95)
]

dist = sns.displot(
    data=data_plot, x="norminc", weights="wgt", kde=True, stat="density", bins=100
).set(
    title="Distribution of annual permanent income (2016 SCF)",
...
data_plot = df2016[["wageinc", "wgt"]][
    df2016["wageinc"] < df2016["wageinc"].quantile(0.95)
]

dist = sns.displot(
    data=data_plot, x="wageinc", weights="wgt", kde=True, stat="density", bins=100
).set(title="Distribution of wage income (2016 SCF)", xlabel="Wage income (USD)")
...
# +
data_plot = df2016[["lqwealth", "wgt"]][
    df2016["lqwealth"] < df2016["lqwealth"].quantile(0.95)
]
...
dist = sns.displot(
    ...
    title="Distribution of net liquid wealth (2016 SCF)", xlabel="Liquid wealth (USD)"
)
...
# +
data_plot = df2016[["lw2income", "wgt"]][
    df2016["lw2income"] < df2016["lw2income"].quantile(0.95)
]
...
dist = sns.displot(
    ...
    title="Distribution of net-liquid-wealth/permanent-income ratio (2016 SCF)",
    xlabel="Liquid wealth/permanent income ratio",
)

lw2income_av = (data_plot["lw2income"] * data_plot["wgt"]).sum() / data_plot[
    "wgt"
].sum()
...
ax.plot(
    SCF_lq_share_agents_ap,
    SCF_lq_share_ap,
    "r-.",
    label="SCF,Gini={}".format(round(gini_lq_SCF, 2)),
)
ax.plot(SCF_lq_share_ap, SCF_lq_share_ap, "k-", label="equality curve")
...
def weighted_percentiles(
    data, variable, weights, percentiles=[], dollar_amt=False, subgroup=None, limits=[]
):
    """
    ...
    if dollar_amt is False:
        return [
            tt.loc[tt[variable] <= a[x], variable + weights].sum()
            / tt[variable + weights].sum()
...
def figureprefs(data, variable="income", labels=False, legendlabels=[]):
    ...
    ax.set_title(
        "Lorenz Curve: United States in 2016",
        fontsize=18,
        loc="center",  # Axes titles
    )
    ...
    if type(data) == list:
        values = [
            weighted_percentiles(
                data[x], variable, "wgt", dollar_amt=False, percentiles=percentiles
            )
        ...
        for index in range(len(data)):
            ...
            for num in [10, 19, 20]:
                ax.annotate(
                    "{:3.1f}%".format(values[index][num] * 100),
        ...
        values = weighted_percentiles(
            data, variable, "wgt", dollar_amt=False, percentiles=percentiles
        )
        plt.plot(
            percentiles,
            values,
    ...
    plt.plot(
        percentiles, percentiles, linestyle="--", color="k", label="Perfect Equality"
    )
...
age_med_lwage_income = df2016.groupby("age").agg(
    med_lwage_income=("lwage_income", "median")
)
...
for df in to_merge:
    SCF_age_profile = pd.merge(SCF_age_profile, df, left_index=True, right_index=True)
