...
def estimate_sample(sample):
    ...
    moms_data = {"Mean": data_mean, "Var": data_var}
    ## initialize
    ...
    ## estimation
    data_para_est = dt_data_est.EstimatePara(method="BFGS", para_guess=para_guess_this)
...
## SIPP data
SIPP_Y = pd.read_stata("../../../SIPP/sipp_matrix_Y2Y.dta", convert_categoricals=False)
...
## time stamp
year_str = [
    string.replace("lwage_id_shk_y2y_gr", "")
    for string in sample_full.columns
    if "lwage_id_shk_y2y_gr" in string
]
...
for i, paras_est in enumerate([data_para_est_full]):
    ...
    plt.plot(years, p_risk, "bv", markersize=20, label="Estimation")
    plt.ylim(0.0, 0.5)
    ...
    plt.plot(years, t_risk, "bv", markersize=20, label="Estimation")
    plt.xticks(years, rotation="vertical")
    plt.ylim(0.0, 0.35)
...
# +
sipp_ests = {
    "YearlyPermanent": np.nanmean(np.sqrt(p_risk)),
...
## SIPP data
SIPP_Q = pd.read_stata("../../../SIPP/sipp_matrix_Q.dta", convert_categoricals=False)
...
## time stamp
quarter_str = [
    string.replace("lwage_Q_id_shk_gr", "")
    for string in sample_full.columns
    if "lwage_Q_id_shk_gr" in string
]
...
for i, paras_est in enumerate([data_para_est_full]):
    ...
    plt.ylabel("std of permanent risk")
    plt.xticks(quarters, rotation="vertical")
    plt.grid(True)
...
# + {"code_folding": []}
## SIPP data
SIPP = pd.read_stata("../../../SIPP/sipp_matrix.dta", convert_categoricals=False)
...
## time stamp
months_str = [
    string.replace("lwage_id_shk_gr", "")
    for string in sample_full.columns
    if "lwage_id_shk_gr" in string
]
...
for i, paras_est in enumerate([data_para_est_full]):
    ...
    plt.subplot(2, 1, 1)
    plt.title("Permanent Risk")
    plt.plot(months[2:], p_risk_mv, "r--o", markersize=8, lw=lw, label="Estimation")
    plt.ylabel("std of permanent risk")
    plt.xticks(months[2::4], rotation="vertical")
    plt.grid(True)

    plt.subplot(2, 1, 2)
    plt.title("Transitory Risk")
    plt.plot(months[2:], t_risk_mv, "r--o", markersize=8, lw=lw, label="Estimation")
    plt.xticks(months[2::4], rotation="vertical")
    plt.ylabel("std of transitory risk")
    plt.legend(loc=0)
    plt.grid(True)
...
for i, para_est in enumerate([data_para_est_full]):
    ...
    # print(group_var)
    times = len(months)
    this_est = pd.DataFrame(
        [list(months), np.abs(para_est[1][0]), np.abs(para_est[1][1])]
    ).transpose()
    est_df = pd.concat([est_df, this_est])
