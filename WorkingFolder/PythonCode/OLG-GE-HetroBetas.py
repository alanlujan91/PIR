...
hetero_beta_types = make_1dtypes("β", beta_types)

# + code_folding=[0]
## stationary distribution of age
from Utility import stationary_age_dist

age_dist_ss = stationary_age_dist(lc_paras["L"], n=0.0, LivPrb=lc_paras["LivPrb"])
# -
...
## h2m share
h2m_share = h2m_ratio(
    results_combined_pe["a_grid_dist"], results_combined_pe["a_pdfs_dist"], 0.5
)

ax.plot(
    np.log(results_combined_pe["ap_grid_dist"] + 1e-5),
...
market_OLG_mkv_this = Market_OLG_mkv_hetero_types(
    households_list=households_by_type, production=production
)
# -
...
def h2m_ratio(a_grid, a_pdfs, cutoff):
    """
    input
    =====
...
## h2m share
h2m_share = h2m_ratio(
    results_combined_ge["a_grid_dist"], results_combined_ge["a_pdfs_dist"], 0.5
)
ax.plot(
    np.log(results_combined_ge["ap_grid_dist"] + 1e-5),
